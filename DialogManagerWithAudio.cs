...
public class DialogManagerWithAudio : MonoBehaviour
{
  ...
  public AudioClip[] button_audios; // 按照顺序的 button 音频
  public AudioSource button_audio_source; // 按钮音频
  ...
  void Awake()
  {
    ...
    button_audio_source.loop = false; // 确保不循环播放
    button_audio_source.playOnAwake = false;
    if (buttons.Length != button_audios.Length)
    {
      Debug.Log($"number of buttons ({buttons.Length}) and button audios ({button_audios.Length}) should match");
    }
  }
  ...
  {
    // 打断原有音频并播放新音频
    if (button_audio_source)
    {
      button_audio_source.Stop();
    }
    button_audio_source.clip = button_audios[button_index];
