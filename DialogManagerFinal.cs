...
public class DialogManagerFinal : MonoBehaviour
{
  ...
  public string next_scene_name;
	...
	// 最后文字的配音（如果需要）
	[SerializeField] public AudioClip finalAudioClip; 
	...
	void Start()
  {
    ...
    {
      finalAudioSource = gameObject.AddComponent<AudioSource>();
    }
  ...
  void game_fail()
  {
    ...
    string final_os = "你的智能小伙伴艾薇星没电啦，快给我充电吧。卡卡宝贝，我们明天再见啦。";

    StartCoroutine(FinalSequence(final_os, lower_text));
  }

  private IEnumerator FinalSequence(string line, TextMeshProUGUI target)
  {
		// 最后的 OS
		yield return StartCoroutine(type_text(line, target, true));
		
		yield return new WaitUntil(() => Input.GetMouseButtonDown(0));
		SceneManager.LoadScene(next_scene_name); // 加载下一场景
  }
  ...
  bool find_key_value_pair(JObject config, string target_key)
  {
    foreach (var pair in config)
    {
      ...
      if (pair.Value is JObject child_obj && find_key_value_pair(child_obj, target_key)) return true;
    }
    return false;
  }
	...
	IEnumerator type_text(string line, TextMeshProUGUI target, bool in_final = false)
	{
		target.text = "";
		System.Text.StringBuilder builder = new System.Text.StringBuilder();

		if (in_final && finalAudioClip != null && builder.Length == 0)
    {
			finalAudioSource.clip = finalAudioClip;
			finalAudioSource.loop = false;
		...
		skip_now = false;
		if (in_final && finalAudioSource != null)
		{
